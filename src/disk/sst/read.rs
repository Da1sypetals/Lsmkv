use super::binary_search::BisectResult;
use super::search::BloomFilter;
use crate::disk::sst::Index;
use crate::disk::sst::binary_search::bisect_index;
...
impl SstReader {
    ...
    pub fn get_by_time(&self, key: &[u8], timestamp: u64) -> Option<Record> {
        ...
        let composite_key = Key::from_slice(key, timestamp);
        match bisect_index(&index, &composite_key) {
            Some(BisectResult::Range(left, right)) => {
                ...
                while pos <= right_offset {
                    // Read record type
                    file.read_exact(&mut type_buf).unwrap();
                        ...
                        */
                        0 => {
                            ...
                            let record_timestamp =
                                u64::from_le_bytes(timestamp_buf.to_vec().try_into().unwrap());

                            // Value record
                            // Read key size
                            file.read_exact(&mut size_buf).unwrap();
                            let key_size =
                                u16::from_le_bytes(size_buf.to_vec().try_into().unwrap()) as usize;
                            ...
                            // left: from disk, right: query
                            if Bytes::copy_from_slice(&buf) < key
                                || &buf == key && record_timestamp > timestamp
                            {
                                // (Sadly Vec<u8> does not implement PartialOrd)
                                // case 1: continue
                                file.seek(std::io::SeekFrom::Current(value_size as i64))
                                    .unwrap();
                            } else if &buf == key && record_timestamp <= timestamp {
                                // Read value
                                // case 2: return value
                                ...
                                // key is greater than the query key
                        ...
                        1 => {
                            ...
                            {
                                // case 1: continue
                                //   essentially do nothing (to skip this value),
                                //   since cursor is at the end of this KV pair.
                            } else if &buf == key && record_timestamp <= timestamp {
                                // Read tomb
                                // case 2: return tomb
    ...
    fn get_index(&self) -> Index {
        ...
        while pos < contents.len() {
            // Read timestamp (u64)
            let timestamp = u64::from_le_bytes(contents[pos..pos + 8].try_into().unwrap());
            pos += 8;
            ...
            index.push((Key::from_slice(&key, timestamp), offset));
        }
