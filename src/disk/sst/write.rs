...
use crate::{
    ...
    memory::{memtable::Memtable, types::Record},
};
use bytes::Bytes;
use std::{
    fs::File,
...
impl SstWriter {
    ...
    /// Build the SST file:
    /// - Write data to disk (in blocks, calls `DataBlockWriter` APIs)
    /// - Write index to disk
    ...
    pub fn build(self) {
        ...
        for (key, datafile_offset) in index {
            ...
            // encode key
            index_writer.write_all(&key.key).unwrap();
